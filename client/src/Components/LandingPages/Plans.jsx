...
const PlanSection = ({ title, items }) => (
  <div>
    <h2 className="text-[4vw] font-[primaryfont] mb-[0.4vw]">
      {title}
    </h2>
      ...
      <div className="w-full h-full flex justify-between items-end">
        {items.map((item, index) => (
          ...
          >
            ...
            >
              {/* CONTENT */}
              <div className="absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2 z-30 text-black px-[2vw] flex flex-col gap-[1vw] text-center animate-fadeIn">
                {hovered === index &&
                  plans[item].map((section, i) => (
                    <PlanSection
                      key={i}
                      title={section.title}
                      items={section.items}
