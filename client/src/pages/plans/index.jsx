...
const Plans = () => {
  ...
  const handleTouchStart = (index, item) => {
    setTouchedItem(touchedItem === index ? null : index);
  };

  useEffect(() => {
    const handleClickOutside = (e) => {
      if (isMobile && touchedItem !== null) {
        const planItems = document.querySelectorAll("[data-plan-item]");
  ...
  const content = {
    FOUNDATION: (
      <div className="text-black px-[2vw] flex flex-col gap-[1.2vw] text-center animate-fadeIn">
        <h2 className="text-[10vw] lg:text-[3vw] font-[primaryfont]">FOUNDATION</h2>
        <p className="text-[4vw] lg:text-[1vw] opacity-80 font-[secondaryregularfont] italic">
          A refined and structured digital base.
        </p>

        {/* Website details */}
        <div>
          <h3 className="text-[6vw] lg:text-[2.3vw] font-[primaryfont]">Website</h3>
          <p className="text-[3vw] lg:text-[0.85vw] opacity-80 leading-[1.35] font-[secondaryregularfont]">
            Elegant, responsive website
            <br />
            Essential, well-crafted pages
            <br />
        ...
        {/* Video Editing & Motion Graphics */}
        <div>
          <h3 className="text-[6vw] lg:text-[2.3vw] font-[primaryfont]">
            Video Editing & Motion Graphics
          </h3>
          <p className="text-[3vw] lg:text-[0.85vw] opacity-80 leading-[1.35] font-[secondaryregularfont]">
            Branding intro/outro clips
        ...
        {/* Automation (n8n) */}
        <div>
          <h3 className="text-[6vw] lg:text-[2.3vw] font-[primaryfont]">Automation (n8n)</h3>
          <p className="text-[3vw] lg:text-[0.85vw] opacity-80 leading-[1.35] font-[secondaryregularfont]">
            Lead capture with alerts
            <br />
            Auto-save to Sheets/CRM
    ...
    ELEVATION: (
      <div className="text-black px-[2vw] flex flex-col gap-[1.2vw] text-center animate-fadeIn">
        <h2 className="text-[10vw] lg:text-[3vw] font-[primaryfont]">ELEVATION</h2>
        <p className="text-[4vw] lg:text-[1vw] opacity-80 font-[secondaryregularfont] italic">
          Strengthening your brand across every platform.
        </p>
    ...
    DOMINANCE: (
      <div className="text-black px-[2vw] flex flex-col gap-[1.2vw] text-center animate-fadeIn">
        <h2 className="text-[10vw] lg:text-[3vw] font-[primaryfont]">DOMINANCE</h2>
        <p className="text-[4vw] lg:text-[1vw] opacity-80 font-[secondaryregularfont] italic">
          A complete digital ecosystem built for scale.
        </p>
  ...
  return (
    <div className="w-full h-full">
      ...
      {!isMobile && (
        <div className="relative w-full h-screen bg-tramsparent flex flex-col justify-between items-center  ">
          {/* Page Title */}
          <h1
            className={`absolute top-0 font-[primaryfont] text-[10vw] text-[#D9F4A0] z-20 ${
              isFoundationHovered || isElevationHovered || isDominanceHovered
          ...
          <div className="w-full flex flex-col z-10">
            {items.map((item, index) => (
              <div
                key={index}
                data-plan-item
                className="w-full h-auto min-h-[100vh] flex justify-center items-center relative z-10 flex-col"
                onClick={() => {
                  if (isMobile) handleTouchStart(index, item);
                }}
              >
                {/* SUNRAY EFFECT */}
                <div
                  className="fixed top-0 left-0 w-full h-full bg-[#D9F4A0] transition-all duration-[800ms] ease-out z-20 overflow-hidden pointer-events-none"
                  style={{
                    clipPath:
                      touchedItem === index
                        ? "polygon(-6% -2%, -10% -4%, 400% 90%, 28% 110%)"
                        : "polygon(0 6%, 0 0, 0 0, 0 0)",
                  }}
                >
                  {/* Content inside sunray */}
                  <div className="absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2 z-30">
                    {touchedItem === index && content[item]}
                  </div>
                </div>
                ...
                {/* MOBILE: Show content below title */}
                {touchedItem === index && (
                  <div className="w-full px-[4vw] py-[4vw] z-10 relative">
                    {content[item]}
                  </div>
                )}
              </div>
